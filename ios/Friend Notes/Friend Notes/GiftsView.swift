    ...
    var body: some View {
        ...
        .sheet(isPresented: editingSheetBinding) {
            if let idea = editingGiftIdea {
                EditGlobalGiftIdeaSheet(idea: idea)
            }
        }
    }
...
private struct AddGlobalGiftIdeaSheet: View {
    ...
    private var assignmentMenu: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(L10n.text("gifts.assignee.label", "Person"))
...
/// Sheet used to edit an existing gift idea including assignment and gifted state.
private struct EditGlobalGiftIdeaSheet: View {
    @Environment(\.dismiss) private var dismiss
    @Query(sort: [SortDescriptor(\Friend.lastName), SortDescriptor(\Friend.firstName)]) private var allFriends: [Friend]
    ...
    var body: some View {
        ...
        .onChange(of: selectedFriendID) { _, newValue in
            idea.friend = allFriends.first(where: { $0.persistentModelID == newValue })
        }
        .sheet(isPresented: $showingAssigneePicker) {
            GiftAssigneePickerSheet(
                allFriends: allFriends,
